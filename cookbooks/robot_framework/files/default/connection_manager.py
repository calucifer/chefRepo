...
class ConnectionManager(object):
    ...
    def connect_to_database(self, dbapiModuleName=None, dbName=None, dbUsername=None, dbPassword=None, dbHost='localhost', dbPort="5432", dbConfigFile="./resources/db.cfg"):
        ...
        if dbapiModuleName in ["MySQLdb", "pymysql"]:
            ...
            logger.debug ('Connecting using : %s.connect(db=%s, user=%s, password=%s, host=%s, port=%s) ' % (dbapiModuleName, dbName, dbUsername, dbPassword, dbHost, dbPort))
            self._dbconnection = db_api_2.connect (db=dbName, user=dbUsername, password=dbPassword, host=dbHost, port=dbPort)
	elif dbapiModuleName in ["cx_Oracle"]:
            dsnStr=db_api_2.makedsn(host=dbHost, port=dbPort, sid=dbName)
	    logger.debug ('Connecting to Oracle db using : %s.connect(db=%s, user=%s, password=%s, host=%s, port=%s, dsnStr=%s) ' % (dbapiModuleName, dbName, dbUsername, dbPassword, dbHost, dbPort, dsnStr))
            self._dbconnection = db_api_2.connect (user=dbUsername, password=dbPassword, dsn=dsnStr)
        else:
            logger.debug ('Connecting using : %s.connect(db=%s, user=%s, password=%s, host=%s, port=%s) ' % (dbapiModuleName, dbName, dbUsername, dbPassword, dbHost, dbPort))
            self._dbconnection = db_api_2.connect (db=dbName, user=dbUsername, password=dbPassword, host=dbHost, port=dbPort)
